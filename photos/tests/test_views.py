...
class PhotographerViewSetTest(APITestCase):
    def setUp(self):
        ...
        self.detail_url = reverse(
            "photos:photographers-detail", kwargs={"pk": self.photographer1.pk}
        )
    ...
    def test_photographer_photos_action_unauthenticated(self):
        url = reverse(
            "photos:photographers-photos", kwargs={"pk": self.photographer1.pk}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)
    ...
    def test_photographer_photos_action_no_photos(self):
        url = reverse(
            "photos:photographers-photos", kwargs={"pk": self.photographer2.pk}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
